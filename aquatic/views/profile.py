...
class ProfileView(View):  # 改繼承純粹的 View，解除強迫登入的物理鎖
    def get(self, request, username=None):
        ...
        # 🚀 新增：計算是否要對這個瀏覽者隱藏價格
        # ────────────────────────────────────────────────────────
        should_hide_price = False  # 物理預設：不隱藏

        # 🎯 因果判斷：如果這個賣家的 profile 有開啟「隱藏價格」開關
        # 注意：請確保你的 Profile model 裡面已經有加 `hide_price_from_non_followers` 這個欄位
        # 🎯 因果判斷：檢查這個賣家的 profile 裡面的 `hide_price` 開關
        if (
            hasattr(profile_owner.profile, "price_visibility")
            and profile_owner.profile.price_visibility
        ):
            # 因：賣家有開防護罩。且現在看的人不是賣家本人，也沒有追蹤賣家
            if not is_me and not is_following:
                # 果：啟動隱藏機制
        ...
        context = {
            ...
            "price_visibility": should_hide_price,  # 🚀 把算好的最終結果丟給前端
        }
...
class UpdateProfileView(FisshPageBase):
    def post(self, request, username, *args, **kwargs):
        ...
        # 修正：使用 price_visibility
        price_visibility = request.POST.get("price_visibility", "PUBLIC")

        if price_visibility not in {"PUBLIC", "FOLLOWERS"}:
